        ...
        <div class="overflow-y-auto flex-1">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-4">
            ...
            <div v-else class="card p-6">
              ...
              <div v-if="tabTenant && tabTenant.regions?.length" class="mt-4 pt-4 border-t border-slate-700">
                ...
                <div class="flex flex-wrap gap-2">
                  <span v-for="region in tabTenant.regions" :key="region" 
            ...
            <div v-if="tabTenant && tabTenant.userList?.length" class="card p-6">
              ...
              <div class="space-y-3">
                <div v-for="user in tabTenant.userList" :key="user.id" 
                  ...
                  <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="flex gap-2 items-center ml-4">
                      ...
                      <span class="text-xs px-2 py-0.5 rounded" 
                            :class="user.state === 'ACTIVE' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'">
                        {{ user.state }}
                      </span>
                    </div>
                  </div>
          ...
          <div v-show="activeTab === 'instances'">
            ...
            <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div v-for="instance in tabInstances" :key="instance.id" 
                ...
                <div class="grid grid-cols-2 gap-2 text-sm mb-4">
                  <div>
                    ...
                    <span class="ml-2 text-white">{{ instance.shape }}</span>
                  </div>
                  <div>
                  ...
                  <div class="col-span-2">
                    ...
                    <span class="ml-2 text-white font-mono text-xs">{{ instance.publicIps?.join(', ') || '无' }}</span>
                  </div>
          ...
          <div v-show="activeTab === 'volumes'">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div v-for="volume in tabVolumes" :key="volume.id"
                   class="border border-slate-600 rounded-lg p-4 hover:border-blue-500/50 transition-all">
                <div class="flex justify-between items-start mb-2">
          ...
          <div v-show="activeTab === 'vcns'">
            ...
            <div v-else class="space-y-3">
              <div v-for="vcn in tabVCNs" :key="vcn.id"
                   class="border border-slate-600 rounded-lg p-4">
                <div class="flex justify-between items-start mb-2">
          ...
          <div v-show="activeTab === 'traffic'">
            <div class="space-y-4">
              ...
              <div class="card p-4">
                ...
                <button @click="loadTrafficData" class="btn btn-primary mt-4" :disabled="loadingTraffic">
...
const submitInstanceTask = async () => {
  ...
  try {
    await api.post('/oci/createInstance', {
...
const viewConfigDetails = async (config) => {
  ...
  try {
    const response = await api.post('/oci/details', {
      configId: config.id
    })
...
const loadInstances = async (clearCache = false) => {
  ...
  try {
    const response = await api.post('/oci/details/instances', {
...
const loadVolumes = async (clearCache = false) => {
  ...
  loadingTab.value = true
  try {
    const response = await api.post('/oci/details/volumes', {
      configId: configDetails.value.userId,
      clearCache
    })
...
const loadVCNs = async (clearCache = false) => {
  ...
  loadingTab.value = true
  try {
    const response = await api.post('/oci/details/vcns', {
      configId: configDetails.value.userId,
      clearCache
    })
...
const loadTenant = async (clearCache = false) => {
  ...
  loadingTab.value = true
  try {
    const response = await api.post('/oci/tenant/info', {
      configId: configDetails.value.userId,
      clearCache
    })
...
const loadTrafficCondition = async () => {
  ...
  try {
    const response = await api.get('/oci/traffic/condition', {
      params: { configId: configDetails.value.userId }
    })
...
const watchActiveTab = () => {
  if (activeTab.value === 'basic' && !tabTenant.value) {
    loadTenant()
  } else if (activeTab.value === 'instances' && tabInstances.value.length === 0) {
    loadInstances()
  } else if (activeTab.value === 'volumes' && tabVolumes.value.length === 0) {
    loadVolumes()
  } else if (activeTab.value === 'vcns' && tabVCNs.value.length === 0) {
    loadVCNs()
  } else if (activeTab.value === 'traffic' && trafficCondition.value.instances.length === 0) {
...
const refreshCurrentTab = async () => {
  ...
  } else if (activeTab.value === 'vcns') {
    await loadVCNs(true)
  } else if (activeTab.value === 'traffic') {
    await loadTrafficCondition()
  }
}
...
const resetUserPassword = async (user) => {
  ...
  try {
    await api.post('/oci/tenant/resetPassword', {
      ociCfgId: configDetails.value.userId,
      userId: user.id
    })
...
const clearUserMfa = async (user) => {
  ...
  try {
    await api.post('/oci/tenant/deleteMfaDevice', {
      ociCfgId: configDetails.value.userId,
      userId: user.id
    })
...
const clearUserApiKeys = async (user) => {
  ...
  try {
    await api.post('/oci/tenant/deleteApiKey', {
      ociCfgId: configDetails.value.userId,
      userId: user.id
    })
...
const deleteUser = async (user) => {
  ...
  try {
    await api.post('/oci/tenant/deleteUser', {
      ociCfgId: configDetails.value.userId,
      userId: user.id
    })
...
const terminateInstanceInDetails = async (instanceId) => {
  ...
  instanceActionLoading[instanceId] = true
  try {
    await api.post('/instance/terminate', {
      userId: configDetails.value.userId,
      instanceId
    })
...
const viewInstances = async (config) => {
  selectedConfigForInstances.value = config
  showInstancesSidebar.value = true
  await loadInstancesForConfig(config)
}

const closeInstancesSidebar = () => {
  showInstancesSidebar.value = false
  selectedConfigForInstances.value = null
  instances.value = []
}

const loadInstancesForConfig = async (config) => {
  loadingInstances.value = true
  try {
    const response = await api.post('/instance/list', {
      userId: config.id,
      compartmentId: config.ociTenantId
    })
...
const refreshInstances = async () => {
  if (selectedConfigForInstances.value) {
    await loadInstancesForConfig(selectedConfigForInstances.value)
  }
}
...
const controlInstance = async (instanceId, action) => {
  ...
  try {
    await api.post(actionMap[action].endpoint, {
      userId: selectedConfigForInstances.value.id,
      instanceId
    })
    toast.success(`${actionMap[action].message}操作已提交`)
